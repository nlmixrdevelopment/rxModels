...
void rxModels_pk3cmt_dydt(int *_neq, double __t, double *__zzStateVar__, double *__DDtStateVar__)
{
  int _cSub = _neq[1];
  ...
  cp=linCmtA(_solveData, _cSub,t,0,3,1,cl,v,q,vp,q2,vp2,popLagDepot*exp(bsvLagDepot),1.0,0.0,0.0,ka,popLagCentral*exp(bsvLagCentral),1.0,popRateCentral*exp(bsvRateCentral),popDurCentral*exp(bsvDurCentral));
  (&_solveData->subjects[_cSub])->dadt_counter[0]++;
}

// Jacobian derived vars
void rxModels_pk3cmt_calc_jac(int *_neq, double __t, double *__zzStateVar__, double *__PDStateVar__, unsigned int __NROWPD__) {
  int _cSub=_neq[1];
  double t = __t + _solveData->subjects[_neq[1]].curShift;
  (void)t;
...
void rxModels_pk3cmt_calc_lhs(int _cSub, double __t, double *__zzStateVar__, double *_lhs) {
...
double rxModels_pk3cmt_F(int _cSub,  int _cmt, double _amt, double __t, double *__zzStateVar__){
 return _amt;
}
// Functional based absorption lag
double rxModels_pk3cmt_Lag(int _cSub,  int _cmt, double __t){
...
double rxModels_pk3cmt_Rate(int _cSub,  int _cmt, double _amt, double __t){
...
double rxModels_pk3cmt_Dur(int _cSub,  int _cmt, double _amt, double __t){
...
void rxModels_pk3cmt_ME(int _cSub, double _t, double __t, double *_mat, const double *__zzStateVar__){
   double t = __t + _solveData->subjects[_cSub].curShift;
  (void)t;
  }
// Inductive linearization Matf
void rxModels_pk3cmt_IndF(int _cSub, double _t, double __t, double *_matf){
   double t = __t + _solveData->subjects[_cSub].curShift;
  (void)t;
  }
extern SEXP rxModels_pk3cmt_model_vars(){
  ...
  if (!_rxIsCurrentC(_mv)){
   ...
   setAttrib(sLinCmt,   R_NamesSymbol, sLinCmtN);
    ...
    SEXP slhs      = PROTECT(allocVector(STRSXP, 7));pro++;
  ...
  SET_STRING_ELT(lhs,0,mkChar("cp"));
    ...
    SET_STRING_ELT(model,0,mkChar("popCl=1;\npopV=20;\npopKa=1;\npopVp=10;\npopQ=2;\npopQ2=2;\npopVp2=100;\nbsvCl=0;\nbsvV=0;\nbsvKa=0;\nbsvVp=0;\nbsvQ=0;\nbsvQ2=0;\nbsvVp2=0;\ncl~popCl*exp(bsvCl);\nv~popV*exp(bsvV);\nka~popKa*exp(bsvKa);\nq~popQ*exp(bsvQ);\nvp~popVp*exp(bsvVp);\nq2~popQ2*exp(bsvQ2);\nvp2~popVp2*exp(bsvVp2);\npopLagDepot=0;\npopLagCentral=0;\npopRateCentral=0;\npopDurCentral=0;\nbsvLagDepot=0;\nbsvLagCentral=0;\nbsvRateCentral=0;\nbsvDurCentral=0;\ncp=linCmtA(rx__PTR__,t,0,3,1,cl,v,q,vp,q2,vp2,popLagDepot*exp(bsvLagDepot),1.0,0.0,0.0,ka,popLagCentral*exp(bsvLagCentral),1.0,popRateCentral*exp(bsvRateCentral),popDurCentral*exp(bsvDurCentral));\n"));
...
SEXP matLst = PROTECT(allocVector(VECSXP, 0));pro++;
 SET_VECTOR_ELT(lst,  17, matLst);
    ...
    SET_STRING_ELT(mmd5,1,mkChar("31dd00eb77d4e4630812ea137a83dba7"));
    ...
    SET_STRING_ELT(tran, 20,mkChar("rxModels_pk3cmt_ME"));
    SET_STRING_ELT(trann,21,mkChar("IndF"));
    SET_STRING_ELT(tran, 21,mkChar("rxModels_pk3cmt_IndF"));
...
extern int rxModels_pk3cmt_dydt_liblsoda(double __t, double *y, double *ydot, void *data)
{
  int *neq = (int*)(data);
  rxModels_pk3cmt_dydt(neq, __t, y, ydot);
  return(0);
}
...
void R_init0_rxModels_pk3cmt(){
  ...
  R_RegisterCCallable("rxModels","rxModels_pk3cmt_ME", (DL_FUNC) rxModels_pk3cmt_ME);
  R_RegisterCCallable("rxModels","rxModels_pk3cmt_IndF", (DL_FUNC) rxModels_pk3cmt_IndF);
